...
def borrarRegistroG():
    ...
    if messagebox.askyesno(message="¿Realmente desea eliminar el registro? Se borrarán los datos y las imágenes", title="ADVERTENCIA"):
        try:
            path_2erase_p = os.getcwd() + "\\gimg\\" + selectedG
            print(path_2erase_p)
            shutil.rmtree(path_2erase_p)
...
def insertar_editables_Gatos():
    ...
    if not valuesG[11] == "N/A":
        split_fep = valuesG[11].split('-')
        fep_date = date(int(split_fep[0]),int(split_fep[1]),int(split_fep[2]))
        date_fecha_esterilizacionG.set_date(fep_date)
    #12 fecha ingreso
    split_fip = valuesG[12].split('-')
    fip_date = date(int(split_fip[0]),int(split_fip[1]),int(split_fip[2]))
    date_fecha_ingresoG.set_date(fip_date)
...
def ventana_archivar_estadoG():
    ...
    lbl_vaep = Label(ventana_archivar_estado_Gatos, text="Motivo:", bg='#0a4369', fg="white",font='Helvetica 20')
    lbl_vaep.pack(pady=10)
    RBestadop = IntVar()
    RBestadop.set(1)
    rb_ep1 = tk.Radiobutton(ventana_archivar_estado_Gatos, text="Adoptado", padx = 5, variable=RBestadop, value=1,font=('Helvetica 14'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
    rb_ep1.pack(pady=10)
    rb_ep2 = tk.Radiobutton(ventana_archivar_estado_Gatos, text="Fallecido", padx = 5, variable=RBestadop, value=2,font=('Helvetica 14'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
    rb_ep2.pack(pady=10)
    btn_aceptar = Button(ventana_archivar_estado_Gatos, text="Aceptar", width=7, font='Helvetica 13 bold', bg='#33ff6d',command=lambda: aceptar(RBestadop))
    btn_aceptar.pack(pady=10)
...
def ventana_buscarGa():
    ...
    lbl_busqueda_formato.grid(column=1, row=2, sticky=W, padx=7, pady=2)
    btn_cancelarBP = Button(ventana_buscar_Gatos_arch, text="Cancelar", width=8, font='Helvetica 11 bold', bg='pink',command=lambda:ventana_buscar_Gatos_arch.destroy())
    btn_cancelarBP.grid(column=0, row=4, sticky=W, padx=5, pady=5)
...
def ventana_buscarG():
    ...
    lbl_busqueda_formato.grid(column=1, row=2, sticky=W, padx=7, pady=2)
    btn_cancelarBP = Button(ventana_buscar_Gatos, text="Cancelar", width=8, font='Helvetica 11 bold', bg='pink',command=lambda:ventana_buscar_Gatos.destroy())
    btn_cancelarBP.grid(column=0, row=4, sticky=W, padx=5, pady=5)
...
def abrir_ventana_Gatos_archivados():
    ...
    fHeader_vpa = tk.Frame(fMainFrame1, bg='#0a4369')
    fHeader_vpa.place(relx=0.01, rely=0.01, relwidth=0.98, relheight=0.1)
    lbl_vpa_Gatos = tk.Label(fHeader_vpa, text="Gatos archivados", font='Helvetica 36 bold', bg='#0a4369', fg='pink').pack(side='left', padx=10)
    btn_vp_Buscar = tk.Button(fHeader_vpa, text="Buscar", font='Helvetica 20 bold', bg='#edd972', command=ventana_buscarGa).pack(side='right', padx=10)
    btn_vp_LimpiarBusqueda = tk.Button(fHeader_vpa, text="Limpiar búsqueda", font='Helvetica 10 bold', bg='#edd972',command=mostrarCamposGa).pack(side='right', padx=10, pady=(30,0))
    ...
    btn_vpa_Regresar = tk.Button(fFooter_vGa, text="Regresar", font='Helvetica 20 bold', bg='#33ff6d', command=lambda:[ventana_Gatos_archivados.destroy(), ventana_Gatos.deiconify(),seleccionarUsandoClickG(treeVG)]).pack(side='right', padx=10)
    btn_vpa_Desarchivar = tk.Button(fFooter_vGa, text="Desarchivar", font='Helvetica 20 bold', bg='#aaf76a', command=lambda:[desarchivarG()]).pack(side='left', padx=10)
    btn_vpa_VerFotos = tk.Button(fFooter_vGa, text="Fotos", font='Helvetica 20 bold', bg='#33ff6d', command = lambda:[crear_ventana_ver_fotos_G(True),ventana_Gatos_archivados.iconify()]).pack(side='left', padx=10)
...
def abrir_ventana_Gatos():
    ...
    fHeader_vp = tk.Frame(fMainFrame1, bg='#0a4369')
    fHeader_vp.place(relx=0.01, rely=0.01, relwidth=0.98, relheight=0.1)
    lbl_vp_Gatos = tk.Label(fHeader_vp, text="Gatos", font='Helvetica 36 bold', bg='#0a4369', fg='pink').pack(side='left', padx=10)
    btn_vp_Agregar = tk.Button(fHeader_vp, text="Agregar", font='Helvetica 20 bold', bg='#33ff6d', command=lambda: agregar_Gatitos(True)).pack(side='right', padx=10)
    btn_vp_Buscar = tk.Button(fHeader_vp, text="Buscar", font='Helvetica 20 bold', bg='#edd972', command=ventana_buscarG).pack(side='right', padx=10)
    btn_vp_LimpiarBusqueda = tk.Button(fHeader_vp, text="Limpiar búsqueda", font='Helvetica 10 bold', bg='#edd972',command=mostrarCamposG).pack(side='right', padx=10, pady=(30,0))
...
def abrir_ventana_ver_fotos_G(archivado):
    ...
    lbl_vfp_n_Gatos = Label(fVerfotos_G_header, text="", font='Helvetica 30 bold', bg='#0a4369', fg='pink')
    lbl_vfp_n_Gatos.pack(side='left', padx=50)
    lbl_vfp_n_Gatos.config(text=valuesG[0])
    lbl_vfp_fi_Gatos = Label(fVerfotos_G_header, text = "",font='Helvetica 15 bold', bg='#0a4369', fg='pink')
    lbl_vfp_fi_Gatos.pack(side = 'right',padx=5)
    lbl_vfp_fi_Gatos.config(text = valuesG[12])
    lbl_vfp_fit_Gatos = Label(fVerfotos_G_header, text = "Fecha de ingreso: ",font='Helvetica 15 bold', bg='#0a4369', fg='pink')
    lbl_vfp_fit_Gatos.pack(side = 'right',padx=5)
...
def crearO(fMainframe2):
    ...
    try:
        ...
        (datosO) = nombreO.get(), fechanacimientoO, sexoO, razaO.get(), colorO.get(), peloO, tallaO, temperamentoO.get(), esterilizacionO, discapacidadO.get(), adoptableO, str(fechaesterilizacionO), str(fechaingresoO)
        cursor.execute("INSERT INTO gatos VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?)", (datosO))
        conexion.commit()
    except:
        messagebox.showwarning("ADVERTENCIA","Ocurrió un error al crear el registro")
        pass
...
def editarO(fMainframe2):
    ...
    try:
        ...
        print(str(fechaesterilizacionO))
        print(str(fechaingresoO))
        datosG = nombreO.get(), fechanacimientoO, sexoO, razaO.get(), colorO.get(), peloO, tallaO, temperamentoO.get(), esterilizacionO, discapacidadO.get(), adoptableO, str(fechaesterilizacionO), str(fechaingresoO)
        cursor.execute("UPDATE gatos SET NOMBRE=?, FECHANACIMIENTO=?, SEXO=?, RAZA=?, COLOR=?, PELO=?, TALLA=?, TEMPERAMENTO=?, ESTERILIZACION=?, DISCAPACIDAD=?, ADOPTABLE=?, FECHAESTERILIZACION=?, FECHAINGRESO=? WHERE ID="+selectedO, (datosG))
        conexion.commit()
    except:
        messagebox.showwarning("ADVERTENCIA","Ocurrió un error al editar el registro")
        pass
...
def mostrarCamposO():
    ...
    try:
        ...
        for row in cursor:
            treeVO.insert("",0,text=row[0], iid=row[0],values=(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13]))
    except:
        pass
...
def borrarRegistroG():
    ...
    if messagebox.askyesno(message="¿Realmente desea eliminar el registro? Se borrarán los datos y las imágenes", title="ADVERTENCIA"):
        try:
            path_2erase_o = os.getcwd() + "\\gimg\\" + selectedO
            print(path_2erase_o)
            shutil.rmtree(path_2erase_o)
            cursor.execute("DELETE FROM gatos WHERE ID="+selectedO)
            conexion.commit()
        except:
            messagebox.showwarning("ADVERTENCIA","Ocurrió un error al tratar de eliminar el registro")
            pass
...
def mostrarCamposOt():
    ...
    try:
        ...
        for row in cursor:
            treeVOt.insert("",0,text=row[0], iid=row[0],values=(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],row[14]))
    except:
        pass
...
def buscarOt():
    ...
    try:
        ...
        if not records: #empty list
            ...
            for record in records:
                treeVOt.insert("",0,text=record[0], iid=record[0],values=(record[1],record[2],record[3],record[4],record[5],record[6],record[7],record[8],record[9],record[10],record[11],record[12],record[13],record[14]))
    except:
        messagebox.showwarning("ADVERTENCIA","Ocurrió un error")
        mostrarCamposOt()
...
def buscarO():
    ...
    try:
        ...
        if not records: #empty list
            ...
            for record in records:
                treeVO.insert("",0,text=record[0], iid=record[0],values=(record[1],record[2],record[3],record[4],record[5],record[6],record[7],record[8],record[9],record[10],record[11],record[12],record[13]))
    except:
        messagebox.showwarning("ADVERTENCIA","Ocurrió un error")
        mostrarCamposO()
...
def insertar_editables_Otros():
    #0 nombre
    e_nombreO.insert(0,valuesO[0])
    #1 fecha nacimiento
    split_fnp = valuesO[1].split('-')
    e_anoNacO.insert(0,split_fnp[0])
    e_mesNacO.insert(0,split_fnp[1])
    #2 sexo
    if valuesO[2] == 'Hembra':
        RBsexoO.set(1)
    elif valuesO[2] == 'Macho':
        RBsexoO.set(2)
    #3 raza
    e_razaO.insert(0,valuesO[3])
    #4 color
    e_colorO.insert(0,valuesO[4])
    #5 pelo
    if valuesO[5] == 'Corto':
        RBpeloO.set(1)
    elif valuesO[5] == 'Largo':
        RBpeloO.set(2)
    elif valuesO[5] == 'Duro':
        RBpeloO.set(3)
    elif valuesO[5] == 'Alambre':
        RBpeloO.set(4)
    elif valuesO[5] == 'Chino':
        RBpeloO.set(5)
    else:
        RBpeloO.set(6)
        e_peloO.insert(0,valuesO[5])
    #6 talla
    if valuesO[6] == 'Chico':
        RBtallaO.set(1)
    elif valuesO[6] == 'Mediano':
        RBtallaO.set(2)
    elif valuesO[6] == 'Grande':
        RBtallaO.set(3)
    #7 temperamento
    e_temperamentoO.insert(0,valuesO[7])
    #8 esterilizacion
    if valuesO[8] == 'Si':
    ...
    #9 discapacidad
    e_discapacidadO.insert(0,valuesO[9])
    #10 adoptable
    if valuesO[10] == 'Si':
        RBadoptableO.set(1)
    elif valuesO[10] == 'No':
        RBadoptableO.set(2)
    #11 fecha esterilizacion
    if not valuesO[11] == "N/A":
        split_fep = valuesO[11].split('-')
        fep_date = date(int(split_fep[0]),int(split_fep[1]),int(split_fep[2]))
        date_fecha_esterilizacionO.set_date(fep_date)
    #12 fecha ingreso
    split_fip = valuesO[12].split('-')
    fip_date = date(int(split_fip[0]),int(split_fip[1]),int(split_fip[2]))
    date_fecha_ingresoO.set_date(fip_date)
...
def ventana_archivar_estadoO():
    ...
    ventana_archivar_estado_Otros.title("PawSystem Gatos Archivar Motivo")
    ...
    RBestadoo = IntVar()
    RBestadoo.set(1)
    rb_ep1 = tk.Radiobutton(ventana_archivar_estado_Otros, text="Adoptado", padx = 5, variable=RBestadoo, value=1,font=('Helvetica 14'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
    rb_ep1.pack(pady=10)
    rb_ep2 = tk.Radiobutton(ventana_archivar_estado_Otros, text="Fallecido", padx = 5, variable=RBestadoo, value=2,font=('Helvetica 14'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
    rb_ep2.pack(pady=10)
...
def ventana_buscarOt():
    ...
    ventana_buscar_Otros_arch.title("PawSystem Gatos Búsqueda")
    ...
    lbl_opcion.grid(column=0, row=0, sticky=W, padx=5, pady=(10,5))
    combo = ttk.Combobox(ventana_buscar_Otros_arch,state="readonly", font='Helvetica 10', values=["Nombre", "Fecha de nacimiento", "Sexo", "Raza", "Color", "Pelo","Talla","Temperamento","Esterilización","Discapacidad","Adoptable","Fecha de esterilización","Fecha de ingreso","Estado"])
    combo.grid(column=1, row=0, sticky=W, padx=10, pady=(10,5))
    ...
    lbl_busqueda_formato.grid(column=1, row=2, sticky=W, padx=7, pady=2)
    btn_cancelarBP = Button(ventana_buscar_Otros_arch, text="Cancelar", width=8, font='Helvetica 11 bold', bg='pink',command=lambda:ventana_buscar_Otros_arch.destroy())
    btn_cancelarBP.grid(column=0, row=4, sticky=W, padx=5, pady=5)
...
def ventana_buscarO():
    ...
    ventana_buscar_Otros.title("PawSystem Gatos Búsqueda")
    ...
    lbl_opcion.grid(column=0, row=0, sticky=W, padx=5, pady=(10,5))
    combo = ttk.Combobox(ventana_buscar_Otros,state="readonly", font='Helvetica 10', values=["Nombre", "Fecha de nacimiento", "Sexo", "Raza", "Color", "Pelo","Talla","Temperamento","Esterilización","Discapacidad","Adoptable","Fecha de esterilización","Fecha de ingreso"])
    combo.grid(column=1, row=0, sticky=W, padx=10, pady=(10,5))
    ...
    lbl_busqueda_formato.grid(column=1, row=2, sticky=W, padx=7, pady=2)
    btn_cancelarBP = Button(ventana_buscar_Otros, text="Cancelar", width=8, font='Helvetica 11 bold', bg='pink',command=lambda:ventana_buscar_Otros.destroy())
    btn_cancelarBP.grid(column=0, row=4, sticky=W, padx=5, pady=5)
...
def abrir_ventana_Otros_archivados():
    ...
    fHeader_vpa = tk.Frame(fMainFrame1, bg='#0a4369')
    fHeader_vpa.place(relx=0.01, rely=0.01, relwidth=0.98, relheight=0.1)
    lbl_vpa_Otros = tk.Label(fHeader_vpa, text="Gatos archivados", font='Helvetica 36 bold', bg='#0a4369', fg='pink').pack(side='left', padx=10)
    btn_vp_Buscar = tk.Button(fHeader_vpa, text="Buscar", font='Helvetica 20 bold', bg='#edd972', command=ventana_buscarOt).pack(side='right', padx=10)
    btn_vp_LimpiarBusqueda = tk.Button(fHeader_vpa, text="Limpiar búsqueda", font='Helvetica 10 bold', bg='#edd972',command=mostrarCamposOt).pack(side='right', padx=10, pady=(30,0))
    ...
    btn_vpa_Regresar = tk.Button(fFooter_vOt, text="Regresar", font='Helvetica 20 bold', bg='#33ff6d', command=lambda:[ventana_Otros_archivados.destroy(), ventana_Otros.deiconify(),seleccionarUsandoClickO(treeVO)]).pack(side='right', padx=10)
    btn_vpa_Desarchivar = tk.Button(fFooter_vOt, text="Desarchivar", font='Helvetica 20 bold', bg='#aaf76a', command=lambda:[desarchivarO()]).pack(side='left', padx=10)
    btn_vpa_VerFotos = tk.Button(fFooter_vOt, text="Fotos", font='Helvetica 20 bold', bg='#33ff6d', command = lambda:[crear_ventana_ver_fotos_O(True),ventana_Otros_archivados.iconify()]).pack(side='left', padx=10)
...
def abrir_ventana_Otros():
    ...
    fHeader_vp = tk.Frame(fMainFrame1, bg='#0a4369')
    fHeader_vp.place(relx=0.01, rely=0.01, relwidth=0.98, relheight=0.1)
    lbl_vp_Otros = tk.Label(fHeader_vp, text="Otros", font='Helvetica 36 bold', bg='#0a4369', fg='pink').pack(side='left', padx=10)
    btn_vp_Agregar = tk.Button(fHeader_vp, text="Agregar", font='Helvetica 20 bold', bg='#33ff6d', command=lambda: agregar_Otros(True)).pack(side='right', padx=10)
    btn_vp_Buscar = tk.Button(fHeader_vp, text="Buscar", font='Helvetica 20 bold', bg='#edd972', command=ventana_buscarO).pack(side='right', padx=10)
    btn_vp_LimpiarBusqueda = tk.Button(fHeader_vp, text="Limpiar búsqueda", font='Helvetica 10 bold', bg='#edd972',command=mostrarCamposO).pack(side='right', padx=10, pady=(30,0))
    ...
    btn_vO_Borrar = tk.Button(fFooter_vO, text="Borrar", font='Helvetica 20 bold', bg='#f03932',command=lambda: borrarRegistroG()).pack(side='left', padx=10)
    ...
    def agregar_Otros(add):
        ...
        lbl_Vagregar_Otros = tk.Label(fagregar_O_header, text="Gatos", font='Helvetica 30 bold', bg='#0a4369', fg='pink').pack(side='left', padx=10)
        ...
        #Labels de categorias
        labels_formulario_Otros(fagregar_O)

        #Calendarios de las categorias
        calendarios_formulario_Otros(fagregar_O)

        #Entradas de las categorias
        entradas_formulario_Otros(fagregar_O)
    ...
    def radiobtns_formulario_Otros(fagregar_O):
        ...
        RBsexoO = IntVar()
        RBsexoO.set(1)
        rb_sG1 = tk.Radiobutton(fagregar_O, text="Hembra", padx = 5, variable=RBsexoO, value=1,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_sG1.grid(row=3,column=1,sticky=W)
        rb_sG2 = tk.Radiobutton(fagregar_O, text="Macho", padx = 5, variable=RBsexoO, value=2,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_sG2.grid(row=3,column=1,sticky=W,padx=120)

        global RBesterilizacionO
        RBesterilizacionO = IntVar()
        RBesterilizacionO.set(1)
        rb_eG1 = tk.Radiobutton(fagregar_O, text="Sí", padx = 10, variable=RBesterilizacionO, value=1,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_eG1.grid(row=9,column=1,sticky=W)
        rb_eG2 = tk.Radiobutton(fagregar_O, text="No", padx = 10, variable=RBesterilizacionO, value=2,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_eG2.grid(row=9,column=1,sticky=W,padx=90)

        global RBadoptableO
        RBadoptableO = IntVar()
        RBadoptableO.set(1)
        rb_aO1 = tk.Radiobutton(fagregar_O, text="Sí", padx = 10, variable=RBadoptableO, value=1,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_aO1.grid(row=11,column=1,sticky=W)
        rb_aO2 = tk.Radiobutton(fagregar_O, text="No", padx = 10, variable=RBadoptableO, value=2,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_aO2.grid(row=11,column=1,sticky=W,padx=90)

        global RBpeloO
        RBpeloO = IntVar()
        RBpeloO.set(1)
        rb_pO1 = tk.Radiobutton(fagregar_O, text="Corto", padx = 10, variable=RBpeloO, value=1,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_pO1.grid(row=6,column=1,sticky=W)
        rb_pO2 = tk.Radiobutton(fagregar_O, text="Largo", padx = 10, variable=RBpeloO, value=2,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_pO2.grid(row=6,column=1,sticky=W,padx=(90,0))
        rb_pO3 = tk.Radiobutton(fagregar_O, text="Duro", padx = 10, variable=RBpeloO, value=3,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_pO3.grid(row=6,column=1,sticky=W,padx=(182,0))
        rb_pO4 = tk.Radiobutton(fagregar_O, text="Alambre", padx = 10, variable=RBpeloO, value=4,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_pO4.grid(row=6,column=1,sticky=W,padx=(267,0))
        rb_pO5 = tk.Radiobutton(fagregar_O, text="Chino", padx = 10, variable=RBpeloO, value=5,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_pO5.grid(row=6,column=1,sticky=W,padx=(377,0))
        rb_pO6 = tk.Radiobutton(fagregar_O, text="Otro:", padx = 10, variable=RBpeloO, value=6,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_pO6.grid(row=6,column=1,sticky=W,padx=(469,0))

        global RBtallaO
        RBtallaO = IntVar()
        RBtallaO.set(1)
        rb_tO1 = tk.Radiobutton(fagregar_O, text="Chico", padx = 5, variable=RBtallaO, value=1,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_tO1.grid(row=7,column=1,sticky=W)
        rb_tO2 = tk.Radiobutton(fagregar_O, text="Mediano", padx = 5, variable=RBtallaO, value=2,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_tO2.grid(row=7,column=1,sticky=W,padx=100)
        rb_tO3 = tk.Radiobutton(fagregar_O, text="Grande", padx = 5, variable=RBtallaO, value=3,font=('Helvetica 12'),bg='#0a4369',fg="white",selectcolor='gray25',activebackground='#0a4369',activeforeground='pink')
        rb_tO3.grid(row=7,column=1,sticky=W,padx=220)

    def entradas_formulario_Otros(fagregar_O):
        global e_razaO
    ...
    def labels_formulario_Otros(fagregar_O):
...
def crear_ventana_ver_fotos_O(archivado):
    ...
    ver_fotos_ven_O.title("PawSystem Otros")
...
def abrir_ventana_ver_fotos_O(archivado):
    ...
    lbl_vfp_n_Otros = Label(fVerfotos_O_header, text="", font='Helvetica 30 bold', bg='#0a4369', fg='pink')
    lbl_vfp_n_Otros.pack(side='left', padx=50)
    lbl_vfp_n_Otros.config(text=valuesO[0])
    lbl_vfp_fi_Otros = Label(fVerfotos_O_header, text = "",font='Helvetica 15 bold', bg='#0a4369', fg='pink')
    lbl_vfp_fi_Otros.pack(side = 'right',padx=5)
    lbl_vfp_fi_Otros.config(text = valuesO[12])
    lbl_vfp_fit_Otros = Label(fVerfotos_O_header, text = "Fecha de ingreso: ",font='Helvetica 15 bold', bg='#0a4369', fg='pink')
    lbl_vfp_fit_Otros.pack(side = 'right',padx=5)
...
def contenido_ver_fotos_O(canvas_ver_foto_O,fverFotos_O_footer,fMainFrame3,archivado,slider,btn_agregarfoto_O,btn_regresar_O,btn_eliminarfotos_O,progbarFO):    
    ...
    images_files = None
    try:
        dir_path_O_fotos = os.getcwd() + "/gimg/" + selectedO #Conseguir directorio de la carpeta de imagenes
        images_files = os.listdir(dir_path_O_fotos)
    except:
        os.makedirs(os.getcwd() + "/gimg/" + selectedO)
        images_files = os.listdir(dir_path_O_fotos)
...
def ventanaAdopcionO():
    ...
    textos = [
        "SU 🎞 #HISTORIA CONTINÚA, en #ADOPCIÒN 🐾Un #rescate que aún sigue vigente"
        " 🇲🇽🐾\nNOMBRE: "+valuesO[0]+"\nNACIÓ: "+valuesO[1]+ "\nTALLA: "+valuesO[6]+"\nSEXO: "
        +valuesO[2]+" #"+valuesO[8]+"estaesterilizado\nTEMPERAMENTO: "+valuesO[7]+"#adopta"
        " #adopta #adoptanocompres #amigoperruno ❤#ADOPCIÓN🇲🇽 #ADOPTA #APADRINA 🧚🏻‍♀"
        " #APADRINAMIENTOVIRTUAL #UnperritogatitoabandonadoenunHOGAR\nPara más información comunícate a:"
        " adopcionesvirtualesomeyocan@yahoo.com.mx #amigoperruno #amigogatuno #perritos #gatitos"
        " #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "¡Conoce a "+valuesO[0]+ "! SU #HISTORIA CONTINUA en #ADOPCION. Nació "+valuesO[1]+" y es un #rescate"
        " que aún sigue vigente, TÚ puedes cambiar su vida\nTALLA: "+valuesO[6]+"\nSEXO:"
        +valuesO[2]+"\n#"+valuesO[8]+"estaesterilizado\nTEMPERAMENTO: "+valuesO[7]+" Con tu ayuda nuestro"
        " #Omeyocanito puede tener una vida mejor y compartir su felicidad y alegría con más personas"
        "\n#adopta #adoptanocompres #amigoperruno ❤ #ADOPCIÓN🇲🇽 #ADOPTA #APADRINA 🧚🏻‍♀ #APADRINAMIENTOVIRTUAL"
        " #UnperritogatitoabandonadoenunHOGAR Para más información comunícate a: adopcionesvirtualesomeyocan@yahoo.com.mx" 
        "\n#amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "¡"+valuesO[0]+"!🐾 sigue aquí con nosotros. El es un #Omeyocanito "+valuesO[7]+". Estaríamos muy" 
        " felices de encontrarle una familia que le dé mucho amor.\nPara más información comunícate a:" 
        " adopcionesvirtualesomeyocan@yahoo.com.m #amigoperruno #amigogatuno #perritos"
        " #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "Adopta a "+valuesO[0]+", lleva un rato con nosotros, pero ya quiere conocer a las personas que serán"
        " su #familia ❤️. Es de talla"+valuesO[6]+" ¡ya es tiempo de darle la vida que merece!" 
        " Si no puedes adoptar puedes #apadrinar a"+valuesO[0]+" o #compartir para que encuentre un hogar"
        " 🐾❤️.\n#ADOPTA #adoptame #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "¡"+valuesO[0]+" merece una oportunidad!. Está con nosotros desde "+valuesO[12]+". Ayúdanos a encontrarle" 
        " una familia. Para más información comunícate a: adopcionesvirtualesomeyocan@yahoo.com.mx #adopta "
        "#amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        ""+valuesO[0]+" es de talla "+valuesO[6]+". Es"+valuesO[7]+", y "+valuesO[2]+""  
        "Si quieres adoptar a "+valuesO[0]+" y que formen una hermosa familia juntos❤️🐾, contáctanos."
        "#ADOPTA#adoptame #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan"
        "adopta #adoptanocompres #amigoperruno❤#ADOPCIÓN🇲🇽 #ADOPTA #APADRINA"
        "🧚🏻‍♀ #APADRINAMIENTOVIRTUAL#UnperritogatitoabandonadoenunHOGAR"
        "Para más información comunícate a adopcionesvirtualesomeyocan@yahoo.com.mx"
        "#amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "Me llamo "+valuesO[0]+" estoy buscando una familia que me de mucho cariño🥰." 
        " Soy "+valuesO[7]+" y de tamaño "+valuesO[6]+". Ayúdame a encontrar la familia que tanto he esperado." 
        " #ADOPTA#adoptame #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan"
        " adopta #adoptanocompres #amigoperruno❤#ADOPCIÓN🇲🇽 #ADOPTA #APADRINA"
        " 🧚🏻‍♀ #APADRINAMIENTOVIRTUAL#UnperritogatitoabandonadoenunHOGAR"
        " Para más información comunícate a adopcionesvirtualesomeyocan@yahoo.com.mx"
        " #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #-------------------------------------------------------------------------------------------------------------------
        "Adopta a "+valuesO[0]+" ya quiere pertenece a una familia, es de talla "+valuesO[6]+"" 
        " Es "+valuesO[7]+". Estamos seguras de que le dará mucho a amor a la familia a la que vaya a pertenecer." 
        " Es muy amigable con otros perros y niños," 
        " pero necesita una familia que pueda brindarle la atención y el tiempo que necesita para mantenerse feliz y saludable."
        " #ADOPTA#adoptame #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan"
        " adopta #adoptanocompres #amigoperruno❤#ADOPCIÓN🇲🇽 #ADOPTA #APADRINA"
        ...
        " #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "Soy "+valuesO[0]+" y soy "+valuesO[6]+"." 
        " Soy "+valuesO[7]+" Necesito una familia que tenga tiempo y amor para dedicar a un perro activo como yo."
        " Si estás interesado en adoptarme, por favor asegúrate de que tienes el tiempo y los recursos necesarios para cuidarme adecuadamente." 
        " Estoy dispuesto a aprender y estoy ansioso por encontrar un hogar lleno de amor. ¡Gracias por considerarme!",
        #--------------------------------------------------------------------------------------------------------------------
        "¡Hola nosotros somos #Omeyocan🖐🏼! Necesitamos tu ayuda🥹, "+valuesO[0]+" 🐶esta buscando una familia con quien compartir su felicidad y cariño ❤️"
        "\nNACIÓ: "+valuesO[1]+"\n"
        "TALLA: "+valuesO[6]+"\n"
        "SEXO: "+valuesO[2]+"\n"
        "#Esterilización "+valuesO[8]+"\n"
        "TEMPERAMENTO: "+valuesO[7]+"\n"
        " #ADOPTA#adoptame #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan"
        " adopta #adoptanocompres #amigoperruno❤#ADOPCIÓN🇲🇽 #ADOPTA #APADRINA"
        " 🧚🏻‍♀ #APADRINAMIENTOVIRTUAL#UnperritogatitoabandonadoenunHOGAR"
        " Para más información comunícate a adopcionesvirtualesomeyocan@yahoo.com.mx"
    ...
    lbl_nombre_O = Label(ventana_adop_O,text="",font='Helvetica 30 bold', fg='pink', bg='#0a4369')
...
def ventanaNoAdopcionO():
    ...
    textos = [
        "¡Hola! Soy "+valuesO[0]+" estoy buscando apoyo para poder tener una vida digna🐾." 
        " No soy adoptable, pero puedes apadrinarme de manera virtual  en adopcionesvirtualesomeyocan@yahoo.com.mx." 
        " No todos podemos ser adoptables por distintas razones, pero siempre existen más maneras de apoyar. ❤️"
        " #APADRINA #noadoptable #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "Existen muchos Omeyocanitos que no tienen la posibilidad de ser adoptados😥 ya que algunos se encuentran" 
        " en rehabilitación física y/o emocional, pero puedes apadrinar.\n" 
        ""+valuesO[0]+"🐾💞 no tiene la posibilidad y busca TU🫵 ayuda para poder tener la vida que merece." 
        " adopcionesvirtualesomeyocan@yahoo.com.mx #APADRINA #noadoptable #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "¡"+valuesO[0]+" te necesita! Aunque 🚫NO es ADOPTABLE🚫 podrías hacer que tenga una mejor vida y pronta rehabilitación ❤‍🩹 ." 
        " En #Omeyocan hay #omeyocanitos que requieren de atención especial y rehabilitación física o emocional, sin embargo," 
        " puedes #apadrinar y contribuir y hacer el cambio 💓. Esta con nosotros desde "+valuesO[12]+". \n"
        "NACIÓ: "+valuesO[1]+"\n"
        "TALLA: "+valuesO[6]+"\n"
        "SEXO: "+valuesO[2]+"\n"
        "#Esterilizado\n"
        ...
        " #UnperritogatitoabandonadoenunHOGAR",
        #--------------------------------------------------------------------------------------------------------------------
        "¡Haz la diferencia en la vida de "+valuesO[0]+"! Apadrina a uno de los adorables animalistos de Omeyocan, un refugio dedicado a cuidar y proteger a los animales." 
        ...
        " #apadrina #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan",
        #--------------------------------------------------------------------------------------------------------------------
        "🚫"+valuesO[0]+" NO ES ADOPTABLE🚫\n"
        " Lleva con nosotros desde "+valuesO[12]+", necesita de tu apoyo para salir adelante,"
        " aunque 🚫NO ES ADOPTABLE🚫 puedes #Apadrinar para ayudar a su cuidado.\n"
        " Para más información comunícate a: adopcionesvirtualesomeyocan@yahoo.com.mx"
        " #apadrina #amigoperruno #amigogatuno #perritos #gatitos #animalitos #Omeyocan"
    ]
    ...
    lbl_nombre_O = Label(ventana_no_adopt_O,text="",font='Helvetica 30 bold', fg='pink', bg='#0a4369')
    lbl_nombre_O.place(relx=0.1, rely=0.02)
    lbl_nombre_O.config(text=valuesO[0])
...
def ventanaDonarO():
    ...
    lbl_nombre_O = Label(donar_O,text="",font='Helvetica 30 bold', fg='pink', bg='#0a4369')
    lbl_nombre_O.place(relx=0.1, rely=0.02)
    lbl_nombre_O.config(text=valuesO[0])
...
def ventanaPublicarO():
    ...
    lbl_nombre_O = Label(ventana_publicar_O,text="",font='Helvetica 40 bold', fg='pink', bg='#0a4369')
...
def carpeta_fotos_O():
    try:
        dir_path_O_fotos = os.getcwd() + "/gimg/" + selectedO
